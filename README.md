<div align="center">

![Header](https://capsule-render.vercel.app/api?type=waving&color=0:0A1128,50:0E6BA8,100:2EC4B6&height=220&section=header&text=OMKAR%20SAPLE&fontSize=48&fontColor=ffffff&desc=Data%20Analytics%20%7C%20Data%20Science%20%7C%20Generative%20AI&descSize=18&descAlignY=58&animation=twinkling)

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=22&pause=1000&colors=2EC4B6,0E6BA8,5DD9C1,C9D6DF&center=true&vCenter=true&width=600&lines=Hi%2C+I'm+Omkar+Saple;Data+Science+%26+ML+Engineer;Building+Real-World+AI%2FML+Systems;Exploring+Generative+AI+%26+LLMs;Turning+Data+Into+Decisions)](https://git.io/typing-svg)

![B.E. Computer Engineering](https://img.shields.io/badge/B.E.-Computer%20Engineering-0E6BA8?style=flat-square&labelColor=0A1128)
![CGPA](https://img.shields.io/badge/CGPA-8.50-0E6BA8?style=flat-square&labelColor=0A1128)
![Location](https://img.shields.io/badge/📍-India-0E6BA8?style=flat-square&labelColor=0A1128)

<a href="https://www.linkedin.com/in/omkarsaple/"><img src="https://img.shields.io/badge/LinkedIn-0E6BA8?style=for-the-badge&logo=linkedin&logoColor=white&labelColor=0A1128"></a>
<a href="mailto:omkarsaple2304@gmail.com"><img src="https://img.shields.io/badge/Email-0E6BA8?style=for-the-badge&logo=gmail&logoColor=white&labelColor=0A1128"></a>
<a href="https://github.com/Omkar-Saple"><img src="https://img.shields.io/badge/GitHub-0E6BA8?style=for-the-badge&logo=github&logoColor=white&labelColor=0A1128"></a>

![Profile Views](https://komarev.com/ghpvc/?username=Omkar-Saple&style=for-the-badge&color=0E6BA8&label=PROFILE+VIEWS)
![Followers](https://img.shields.io/github/followers/Omkar-Saple?style=for-the-badge&color=0E6BA8&labelColor=0A1128)
![Stars](https://img.shields.io/github/stars/Omkar-Saple?style=for-the-badge&color=0E6BA8&labelColor=0A1128)

</div>

---

## 🧠 About Me

I'm a Computer Engineering student focused on **Data Analytics, Data Science, and Generative AI**. I build end-to-end data systems — from ETL pipelines and database design to machine learning models and interactive dashboards — with an emphasis on turning raw, messy data into decisions people can act on. My recent work spans **ML-based security systems** and **sports analytics platforms**, and I'm currently deepening my expertise in **LLMs, RAG pipelines, and AI agents**.

**🎯 Open To:** Data Analyst / Data Scientist roles · Generative AI Engineering roles · Research collaborations · Open-source contributions

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0A1128,100:2EC4B6&height=50&animation=twinkling" width="100%"/>

## 🛠️ Tech Stack

**Languages**
[![My Skills](https://skillicons.dev/icons?i=py,java,mysql)](https://skillicons.dev)

**Data & ML**
![Pandas](https://img.shields.io/badge/Pandas-0E6BA8?style=flat-square&logo=pandas&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-0E6BA8?style=flat-square&logo=numpy&logoColor=white)
![Scikit--learn](https://img.shields.io/badge/Scikit--learn-0E6BA8?style=flat-square&logo=scikit-learn&logoColor=white)
![TensorFlow](https://img.shields.io/badge/TensorFlow-0E6BA8?style=flat-square&logo=tensorflow&logoColor=white)
![XGBoost](https://img.shields.io/badge/XGBoost-0E6BA8?style=flat-square)

**Visualization**
![Power BI](https://img.shields.io/badge/Power%20BI-0E6BA8?style=flat-square&logo=powerbi&logoColor=white)
![Tableau](https://img.shields.io/badge/Tableau-0E6BA8?style=flat-square&logo=tableau&logoColor=white)
![Matplotlib](https://img.shields.io/badge/Matplotlib-0E6BA8?style=flat-square)

**Tooling & Platforms**
[![My Skills](https://skillicons.dev/icons?i=git,vscode)](https://skillicons.dev)
![Streamlit](https://img.shields.io/badge/Streamlit-0E6BA8?style=flat-square&logo=streamlit&logoColor=white)
![Jupyter](https://img.shields.io/badge/Jupyter-0E6BA8?style=flat-square&logo=jupyter&logoColor=white)

---


<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0A1128,100:2EC4B6&height=50&animation=twinkling" width="100%"/>

## 🚀 Featured Projects

<details>
<summary><b>📊 IPL-PowerBI-Dashboard — Cricket Analytics Dashboard</b></summary>
<br>

Interactive Power BI dashboard analyzing IPL match and player data, built with custom DAX measures to surface performance KPIs, team comparisons, and historical trends.

| Aspect | Detail |
|---|---|
| **Stack** | Power BI, DAX, Excel/CSV data sources |
| **Scale** | Multi-season IPL match and player dataset |
| **Core Feature** | Custom DAX measures for KPIs (win %, player stats, team performance) |
| **Impact** | Visual dashboard for exploring champions, runners-up, and season trends |
| **Repository** | [IPL-PowerBI-Dashboard](https://github.com/Omkar-Saple/IPL-PowerBI-Dashboard) |

</details>

<details>
<summary><b>🔐 ML-Based Network Intrusion Detection System</b></summary>
<br>

Real-time system for detecting malicious network traffic using machine learning on network flow characteristics.

| Aspect | Detail |
|---|---|
| **Stack** | Python, Scikit-learn, Flow-based feature extraction |
| **Scale** | Real-time traffic classification pipeline |
| **Security** | Detects intrusion patterns via duration, packet length, inter-arrival time features |
| **Impact** | Live web dashboard for continuous threat monitoring |
| **Repository** | [ML_NIDS](https://github.com/Omkar-Saple/ML_NIDS) |

</details>

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0A1128,100:2EC4B6&height=50&animation=twinkling" width="100%"/>

## 💼 Experience

**AI Intern** — TechSaksham (Microsoft & SAP CSR Initiative, Edunet Foundation)
*(Add exact dates)*

Completed a structured AI/ML internship program combining applied machine learning training with project-based execution.

- Worked on applied ML problem-solving under an industry-aligned curriculum
- Built and evaluated ML models as part of program deliverables
- Collaborated within a CSR-backed learning cohort structure

`Machine Learning` `Python` `Applied AI`

---

## 📜 Certifications

**Codebasics.io**

![GenAI Bootcamp](https://img.shields.io/badge/GenAI%20%26%20Data%20Science%20Bootcamp%203.0-0E6BA8?style=for-the-badge&labelColor=0A1128)
![SQL Advanced](https://img.shields.io/badge/SQL%3A%20Beginner%20to%20Advanced-0E6BA8?style=for-the-badge&labelColor=0A1128)

> *Add AWS / Oracle / NPTEL / Cisco badges here as you complete those certifications — keeping this section to verified credentials only.*

---

## 📊 GitHub Analytics

<div align="center">

![](https://github-readme-stats.vercel.app/api?username=Omkar-Saple&show_icons=true&hide_border=true&count_private=true&bg_color=0A1128&title_color=2EC4B6&icon_color=2EC4B6&text_color=C9D6DF)
![](https://github-readme-stats.vercel.app/api/top-langs/?username=Omkar-Saple&layout=compact&hide_border=true&bg_color=0A1128&title_color=2EC4B6&text_color=C9D6DF)

![](https://streak-stats.demolab.com?user=Omkar-Saple&hide_border=true&background=0A1128&ring=2EC4B6&fire=2EC4B6&currStreakLabel=2EC4B6&sideLabels=C9D6DF&currStreakNum=C9D6DF&sideNums=C9D6DF&dates=6E90A3)

</div>


## 📈 Contribution Activity

<div align="center">

![](https://github-readme-activity-graph.vercel.app/graph?username=Omkar-Saple&hide_border=true&bg_color=0A1128&color=2EC4B6&line=2EC4B6&point=C9D6DF&area_color=0E6BA8&area=true)

</div>

## 🐍 Contribution Snake

<div align="center">

<img src="https://raw.githubusercontent.com/Omkar-Saple/Omkar-Saple/output/github-contribution-grid-snake-dark.svg" />

</div>

---

## 🎯 Current Focus

```yaml
learning:
  - Large Language Models & RAG architectures
  - AI agent frameworks
  - Advanced statistical modeling

building:
  - CricIntel (cricket analytics platform)
  - Personal GenAI project portfolio

exploring:
  - Prompt engineering
  - Vector databases & retrieval systems

open_to:
  - Data Analyst / Data Scientist roles
  - Generative AI Engineering roles
  - Open-source collaboration
```

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0A1128,100:2EC4B6&height=50&animation=twinkling" width="100%"/>

## 📬 Connect

<div align="center">

<a href="mailto:omkarsaple2304@gmail.com"><img src="https://img.shields.io/badge/Gmail-0E6BA8?style=for-the-badge&logo=gmail&logoColor=white&labelColor=0A1128"></a>
<a href="https://www.linkedin.com/in/omkarsaple/"><img src="https://img.shields.io/badge/LinkedIn-0E6BA8?style=for-the-badge&logo=linkedin&logoColor=white&labelColor=0A1128"></a>
<a href="https://github.com/Omkar-Saple"><img src="https://img.shields.io/badge/GitHub-0E6BA8?style=for-the-badge&logo=github&logoColor=white&labelColor=0A1128"></a>

</div>

---

<div align="center">

*"Data tells the story — models predict the ending — I build the systems that connect the two."*

![Footer](https://capsule-render.vercel.app/api?type=waving&color=0:2EC4B6,50:0E6BA8,100:0A1128&height=120&section=footer&animation=twinkling)

</div>



